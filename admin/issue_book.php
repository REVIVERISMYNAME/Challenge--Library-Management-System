...
<div style="min-height: 700px;">
    ...
    <ol>
        ...
        <li><a href="issue_book.php">Issue Book Management</a></li>
        <li>Issue New Book</li>
    </ol>
    <div>
        <div>
            <?php 
            ...
            <div>
                ...
                <div>
                    <form method="post">
                        ...
                        <div>
                            <input type="submit" name="issue_book_button" value="Issue" />
                        </div>  
                    </form>
                    <script>
                    ...
                    book_id.onkeyup = function()
                    {
                        if(this.value.length > 2)
                        {
                            ...
                            fetch('action.php', {
                                ...
                                var html = '<div style="position:absolute; width:93%">';

                                if(responseData.length > 0)
                                {
                                    for(var count = 0; count < responseData.length; count++)
                                    {
                                        html += '<a href="#"><span onclick="get_text(this)">'+responseData[count].isbn_no+'</span> - <span class="text-muted">'+responseData[count].book_name+'</span></a>';
                                    }
                                }
                                else
                                {
                                    html += '<a href="#">No Book Found</a>';
                                }
                        ...
                        {   
                            ...
                            fetch('action.php', {
                                ...
                                if(responseData.length > 0)
                                {
                                    for(var count = 0; count < responseData.length; count++)
                                    {
                                        html += '<a href="#"><span onclick="get_text1(this)">'+responseData[count].user_unique_id+'</span> - <span class="text-muted">'+responseData[count].user_name+'</span></a>';
                                    }
                                }
                                else
                                {
                                    html += '<a href="#">No User Found</a>';
                                }
                                html += '</div>';
            ...
            if($issue_book_id > 0)
            {
                ...
                foreach($result as $row)
                {
                    ...
                    foreach($user_result as $user_data)
                    {
                        ...
                        <table>
                            ...
                            <tr>
                                ...
                                <td width="70%"><img src="'.base_url().'upload/' . $user_data["user_profile"].'" width="100" /></td>
                            </tr>
                        </table>
                        <br />
                        ';
                    ...
                    if($status == "Issue")
                    {
                        ...
                        <br />
                        <div>
                            <input type="submit" name="book_return_button" value="Book Return"/>
                        </div>
                        ';
                    }
    ...
    if(isset($_GET['msg']))
    {
        if($_GET['msg'] == 'add')
        {
            echo '<div role="alert">New Book Issue Successfully<button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>';
        }

        if($_GET["msg"] == 'return')
        {
            echo '
            <div role="alert">Issued Book Successfully Return into Library <button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>
            ';
        }
    }
    ?>
    ...
    <div>
    	<div>
    		<div>
    			<div>
    				<i></i> Issue Book Management
                </div>
                <div align="right">
                    <a href="issue_book.php?action=add">Add</a>
                </div>
            </div>
        </div>
        <div>
        		...
        		if($statement->rowCount() > 0)
        		{
        			...
        			foreach($statement->fetchAll() as $row)
        			{
        				...
        				<tr>
        					...
        					<td>'.$status.'</td>
        					<td>
                                <a href="issue_book.php?action=view&code='.convert_data($row["issue_book_id"]).'">View</a>
                            </td>
        				</tr>
        				';
        			}
