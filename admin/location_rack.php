...
<div style="min-height: 700px;">
	...
	<ol >
		...
		<li><a href="category.php">Location Rack Management</a></li>
		<li>Add Location Rack</li>
	</ol>
	...
	<div>
		<div>
			...
			{
				echo '
				<div role="alert"><ul class="list-unstyled">'.$error.'</ul> <button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>
				';
			}
			...
			<div>
				<div>
					<i></i> Add New Location Rack
                </div>
                <div>
                	<form method="post">
                		<div>
                			<label>Location Rack Name</label>
                			<input type="text" name="location_rack_name" id="location_rack_name"/>
                		</div>
                		<div>
                			<input type="submit" name="add_location_rack"value="Add" />
                		</div>
                	</form>
                </div>
            </div>
	...
	<ol >
		<li><a href="index.php">Dashboard</a></li>
        <li><a href="location_rack.php">Location Rack Management</a></li>
        <li>Edit Location Rack</li>
    </ol>
    <div>
    	<div>
    		<div>
    			<div>
    				<i></i> Edit Location Rack Details
                </div>
                <div>
                	<form method="post">
                		<div>
                			<label>Location Rack Name</label>
                			<input type="text" name="location_rack_name" id="location_rack_name"value="<?php echo $location_rack_row["location_rack_name"]; ?>" />
                		</div>
                		<div>
                			<input type="hidden" name="location_rack_id" value="<?php echo $_GET['code']; ?>" />
                			<input type="submit" name="edit_location_rack" value="Edit" />
                		</div>
                	</form>
                </div>
            </div>
	...
	<ol >
		<li><a href="index.php">Dashboard</a></li>
		<li>Location Rack Management</li>
	</ol>
		<?php 
		...
		if(isset($_GET["msg"]))
		{
			if($_GET["msg"] == 'add')
			{
				echo '<div role="alert">New Location Rack Added<button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}

			if($_GET["msg"] == 'edit')
			{
				echo '<div role="alert">Location Rack Data Edited <button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}

			if($_GET["msg"] == 'disable')
			{
				echo '<div role="alert">Location Rack Status Change to Disable <button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}

			if($_GET["msg"] == 'enable')
			{
				echo '<div role="alert">Location Rack Status Change to Enable <button type="button" data-bs-dismiss="alert" aria-label="Close"></button></div>';
			}
		}
	...
	<div>
		<div>
			<div>
				...
				<div align="right">
					<a href="location_rack.php?action=add">Add</a>
				</div>
			</div>
		</div>
		<div>
			<table id="datatablesSimple">
				...
				if($statement->rowCount() > 0)
				{
					foreach($statement->fetchAll() as $row)
					{
						...
						{
							$location_rack_status = '<div>Enable</div>';
						}
						else
						{
							$location_rack_status = '<div>Disable</div>';
						}
						...
						<tr>
							...
							<td>
								<a href="location_rack.php?action=edit&code='.convert_data($row["location_rack_id"]).'">Edit</a>
								<button type="button" name="delete_button" onclick="delete_data(`'.$row["location_rack_id"].'`, `'.$row["location_rack_status"].'`)">Delete</button>
							</td>
						</tr>
						';
